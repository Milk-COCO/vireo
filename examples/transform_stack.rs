...
fn main() {
    ...
    app.run(move |app| {
        ...
        draw_rectangle(&mut batch, -40.0, -12.0, 80.0, 24.0, RED);
        draw_circle(&mut batch, 40.0, 0.0, 8.0, YELLOW);
        batch.clear_transform();
        draw_text(
            &mut batch.texts,
        ...
        draw_rectangle(&mut batch, 0.0, -10.0, 100.0, 20.0, GREEN);
        draw_circle(&mut batch, 0.0, 0.0, 12.0, Color::new(0.2, 0.8, 0.4, 1.0));
        // 小臂：在大臂末端局部空间继续叠加
        batch.translate(100.0, 0.0);
        batch.rotate_rad(t * 1.4);
        batch.scale_by(0.85 + 0.15 * t.sin(), 1.0);
        draw_rectangle(&mut batch, 0.0, -8.0, 70.0, 16.0, Color::new(0.3, 0.9, 0.7, 1.0));
        draw_circle(&mut batch, 70.0, 0.0, 10.0, Color::new(0.2, 0.9, 0.95, 1.0));
        batch.clear_transform();
        draw_text(
            &mut batch.texts,
        ...
        draw_rounded_rect(&mut batch, -50.0, -40.0, 100.0, 80.0, 12.0, PURPLE);
        draw_rect_outline(&mut batch, -50.0, -40.0, 100.0, 80.0, 2.0, WHITE);
        batch.clear_transform();
        draw_text(
            &mut batch.texts,
            "apply_matrix (shear)",
        ...
        for (x, y) in [(150.0, 220.0), (450.0, 280.0), (720.0, 220.0)] {
            draw_circle_outline(&mut batch, x, y, 4.0, 1.5, Color::new(1.0, 1.0, 1.0, 0.35), 16);
        }
