...
fn main() {
    ...
    app.run(move |app| {
        ...
        if has_mouse {
            draw_circle(&mut batch, cx, cy, 20.0, RED, 32);
            draw_line(&mut batch, cx, 0.0, cx, h, 1.0, Color::new(0.25, 0.25, 0.35, 0.4));
            draw_line(&mut batch, 0.0, cy, w, cy, 1.0, Color::new(0.25, 0.25, 0.35, 0.4));
        ...
        let mut batch = DrawBatch::new();
        if has_mouse {
            draw_rectangle(&mut batch, mouse.0 - 16.0, mouse.1 - 1.0, 32.0, 2.0, WHITE);
            draw_rectangle(&mut batch, mouse.0 - 1.0, mouse.1 - 16.0, 2.0, 32.0, WHITE);
            draw_circle(&mut batch, mouse.0, mouse.1, 8.0, RED, 32);
        }
