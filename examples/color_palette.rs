...
fn main() {
    ...
    app.run(move |app| {
        ...
        for (i, c) in swatches.iter().enumerate() {
            ...
            draw_rounded_rect(&mut batch, x, 60.0, 56.0, 56.0, 8.0, *c);
            draw_text(
                &mut batch.texts,
                &c.to_hex(),
                TextOptions::default()
        ...
        for (i, h) in hexes.iter().enumerate() {
            ...
            let x = 30.0 + i as f32 * 70.0;
            draw_rounded_rect(&mut batch, x, 170.0, 56.0, 56.0, 8.0, c);
            draw_text(
                &mut batch.texts,
                *h,
                TextOptions::default()
        ...
        for i in 0..steps {
            ...
            let c = a.lerp(&b, u);
            draw_rectangle(&mut batch, 30.0 + i as f32 * 18.0, 280.0, 16.0, 40.0, c);
        }
        // 动画 alpha
        let pulse = a.with_alpha(0.3 + 0.7 * (0.5 + 0.5 * t.sin()));
        draw_circle(&mut batch, 560.0, 300.0, 36.0, pulse);
        draw_text(
            &mut batch.texts,
