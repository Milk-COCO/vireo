...
## 前言：别用 Vireo
此项目 95%（**甚至更多**）内容由 **AI生成**。而唯一开发者看过的内容只有 5%（**甚至更少**）。

即使代码经过了 AI 的反复审查，依旧存在随时爆炸的风险。
...
**注意**：虽然这个 `fn main` 看起来是在 `main` 线程上跑，但和 `macroquad` 不同，实际上他是跑在一个叫 `vireo-main` 的线程上，**不是主线程**。具体你可以看这个宏的文档与定义。
