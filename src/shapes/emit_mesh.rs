...
pub(super) fn shape_uv(
    uv: &crate::render::UvRect,
    px: f32,
    py: f32,
    ...
    let u = if max_x > min_x {
        (px - min_x) / (max_x - min_x)
    } else {
        0.5
    };
    let v = if max_y > min_y {
        (py - min_y) / (max_y - min_y)
    } else {
        0.5
    };
...
pub(super) fn join_arc(prev: (f32, f32), at: (f32, f32), next: (f32, f32)) -> (f32, f32) {
    ...
    } else if span < -std::f32::consts::PI {
        span += std::f32::consts::TAU;
    }
    (sa, sa + span)
}
...
pub(super) fn emit_rectangle(batch: &mut DrawBatch, w: f32, h: f32, color: Color) {
    if !w.is_finite() || !h.is_finite() || w <= 0.0 || h <= 0.0 || color.a == 0.0 {
        return;
    }
    match batch.sdf_feather {
        None => {
            ...
            for (px, py, u, v) in &[
                ...
                batch
                    .vertices
                    .push(Vertex::new_uv_xform(*px, *py, *u, *v, color, idx));
            }
            batch
                .indices
...
pub(super) fn emit_circle(batch: &mut DrawBatch, r: f32, color: Color) {
    if !r.is_finite() || r <= 0.0 || color.a == 0.0 {
        return;
    }
    match batch.sdf_feather {
        None => {
            ...
            batch
                .vertices
                .push(Vertex::new_uv_xform(0.0, 0.0, cu, cv, color, idx));
            for i in 0..=n {
            ...
            for i in 0..n {
                batch
                    .indices
                    .extend_from_slice(&[base, base + 1 + i, base + 1 + i + 1]);
            }
        }
        Some(f) => {
            batch.note_sdf();
...
pub(super) fn emit_line(
    ...
    if !thickness.is_finite() || thickness <= 0.0 || color.a == 0.0 {
        return;
    }
...
pub(super) fn emit_ellipse(batch: &mut DrawBatch, rx: f32, ry: f32, color: Color) {
    if !rx.is_finite() || !ry.is_finite() || rx <= 0.0 || ry <= 0.0 || color.a == 0.0 {
        return;
    }
    match batch.sdf_feather {
        None => {
...
pub(super) fn emit_rounded_rect(batch: &mut DrawBatch, w: f32, h: f32, radius: f32, color: Color) {
    if !w.is_finite() || !h.is_finite() || w <= 0.0 || h <= 0.0 || color.a == 0.0 {
        return;
    }
    ...
    match batch.sdf_feather {
        ...
        None => {
            ...
            if x2 > xr && y2 > yr {
                ...
                batch
                    .vertices
                    .push(Vertex::new_uv_xform(xr, yr, u0, v0, color, idx));
                batch
                    .vertices
                    .push(Vertex::new_uv_xform(x2, yr, u1, v1, color, idx));
                batch
                    .vertices
                    .push(Vertex::new_uv_xform(x2, y2, u2, v2, color, idx));
                batch
                    .vertices
                    .push(Vertex::new_uv_xform(xr, y2, u3, v3, color, idx));
            ...
            for &(ex, ey, ew, eh) in &[
                ...
                batch
                    .vertices
                    .push(Vertex::new_uv_xform(ex, ey, u0, v0, color, idx));
                batch
                    .vertices
                    .push(Vertex::new_uv_xform(ex2, ey, u1, v1, color, idx));
                batch
                    .vertices
                    .push(Vertex::new_uv_xform(ex2, ey2, u2, v2, color, idx));
                batch
                    .vertices
                    .push(Vertex::new_uv_xform(ex, ey2, u3, v3, color, idx));
                batch.indices.extend_from_slice(&[
                    base,
                    base + 1,
                    base + 2,
            ...
            for &(cx, cy, sa, ea) in &[
                ...
                batch
                    .vertices
                    .push(Vertex::new_uv_xform(cx, cy, cu, cv, color, idx));
                for i in 0..=cs {
...
pub(super) fn emit_triangle(
    ...
    if l2ab < 0.000001 || l2bc < 0.000001 || l2ca < 0.000001 {
        return;
    }
    ...
    match batch.sdf_feather {
        None => {
            ...
            batch
                .vertices
                .push(Vertex::new_uv_xform(x1, y1, u1, v1, color, idx));
            batch
                .vertices
                .push(Vertex::new_uv_xform(x2, y2, u2, v2, color, idx));
            batch
                .vertices
                .push(Vertex::new_uv_xform(x3, y3, u3, v3, color, idx));
...
pub(super) fn emit_circle_outline(
    batch: &mut DrawBatch,
    r: f32,
    thickness: f32,
    color: Color,
...
pub(super) fn emit_ellipse_outline(
    batch: &mut DrawBatch,
    rx: f32,
    ry: f32,
    thickness: f32,
    ...
    match batch.sdf_feather {
        None => {
            ...
            for i in 0..seg_count {
                ...
                batch
                    .vertices
                    .push(Vertex::new_uv_xform(p0.0, p0.1, u0.0, u0.1, color, idx));
                batch
                    .vertices
                    .push(Vertex::new_uv_xform(p1.0, p1.1, u1.0, u1.1, color, idx));
                batch
                    .vertices
                    .push(Vertex::new_uv_xform(p2.0, p2.1, u2.0, u2.1, color, idx));
                batch
                    .vertices
                    .push(Vertex::new_uv_xform(p3.0, p3.1, u3.0, u3.1, color, idx));
                batch.indices.extend_from_slice(&[
                    base,
                    base + 1,
                    base + 2,
            ...
            for i in 0..vcount {
                ...
                for k in 0..m {
                    batch
                        .indices
                        .extend_from_slice(&[base, base + 1 + k, base + 1 + k + 1]);
                }
            }
        }
        Some(f) => {
